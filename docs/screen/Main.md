...
| 文字起こし本文を右クリック | SeeDraft の本文メニューを表示し、選択範囲または本文全体にコピー、整形、翻訳、抽出、ノート作成を実行。整形/翻訳は対象文字列を置換し、抽出は結果をクリップボードへコピー |
| 文字起こし本文を `Shift` + 右クリック | ブラウザ標準の右クリックメニューを表示 |
...
| ライブ字幕画面で `Esc` を押下 | ライブ字幕画面を閉じ、メイン画面へ戻る |
| 保存済みライブ字幕セッションの `プロジェクト化` を押下 | セッション名でプロジェクトを作成し、各セグメントを1件のノートとして追加 |
