...
- マイク録音、PC上で再生されている音声のキャプチャ、音声ファイルのドラッグ＆ドロップ、Windows のどこからでも使える長押しディクテーションに対応。
- 対応拡張子は `wav`, `mp3`, `m4a`, `mp4`, `webm`, `ogg`, `flac`, `aac`, `opus` です。
- 「モデル」タブから Whisper 音声認識モデルを選択できます。既定モデルは `whisper-small` です。
...
- 必要に応じてバックエンドで Symphonia による音声正規化を行い、マイク録音はブラウザ側で WAV に変換します。PC音声は Windows WASAPI loopback で取得します。
...
- マイク音声またはPC上で再生されている音声を連続バッファし、短い無音を検出して発話単位で文字起こしします。
- セッション中に同時翻訳を有効化できます。
...
初回利用時、SeeDraft は音声認識とテキスト処理で利用できる互換モデルがそれぞれ 1 つ以上ダウンロード済みかを確認します。足りないカテゴリがある場合は、そのカテゴリの既定モデル（音声認識は `whisper-small`、テキスト処理は `qwen2.5-coder-0.5b`）のダウンロードを確認します。SeeDraft は Foundry Local の既定モデルキャッシュを使用し、保存先を上書きしません。
...
[プライバシーポリシー](docs/legal/privacy-policy.md) を参照してください。
